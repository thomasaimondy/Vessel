...
def train_func():
	...
	#########################################
	config.set('experiment name', 'name', 'train') # the model saved
...
def test_func():
	...
	config.set('experiment dir', 'dir', datarootpath) # the model saved
	config.set('testing settings', 'nohup','False')
	os.system('python3 ./retina-unet/run_testing.py')
...
for i in range(4):
	...
	if (test):
		...
		for i in range(int(samples/patches)+1):
			...
			with open(r'./utils/recog_config.txt', 'w') as configfile:
				config.write(configfile)
			img_hdf_func()
