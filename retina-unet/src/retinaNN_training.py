...
config.read('/media/zhangyue/80253940-da51-443b-af59-03e8ba01ec83/zhangyue/thomascodes/Vessel(code)/retina-unet/configuration.txt')
...
if os.path.exists(name_dir+name_experiment):
    pass
elif sys.platform=='win32':
    os.system('md ' + name_dir+name_experiment)
else:
    os.system('mkdir -p ' + name_dir+name_experiment)
...
#============  Training ==================================
checkpointer = ModelCheckpoint(filepath=name_dir+name_experiment+'/'+name_experiment +'_best_weights.h5', verbose=1, monitor='val_loss', mode='auto', save_best_only=True) #save at each epoch if the validation decreased
...
model.save_weights(name_dir+name_experiment+'/'+name_experiment +'_last_weights.h5', overwrite=True)
#test the model
